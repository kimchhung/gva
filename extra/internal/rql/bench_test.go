...
var p2 = MustNewParser(Config{
	Model:        T{},
	ColumnFn:     PascalToCamelCase,
	ColumnNameFn: CamelCaseToSnakeCase,
})

func BenchmarkLargeQueryP2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, err := p2.Parse([]byte(`{
		"filter": {
			"admin": true,
			...
			"$or": [
				...
				{ "name": { "$like": "foo" } },
				{ "workName": { "$like": "bar" } },
				{ "workAddressPtrString": { "$like": "baz" } }
			],
			"createdAt": "2018-05-10T05:03:31.031Z",
			"int": 10,
			"nullInt": 10,
			"date": "2018-05-10T05:03:31.031Z",
			"bool": true,
...
func BenchmarkMediumCamecase(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, err := p2.Parse([]byte(`{
			"filter": {
				"name": "foo",
...
func BenchmarkSmallWithCamecaseQuery(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, err := p2.Parse([]byte(`{
		"filter": {
			"createdAt": "2018-05-10T05:03:31.031Z",
