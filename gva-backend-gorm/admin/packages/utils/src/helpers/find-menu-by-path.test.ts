...
const menus: any[] = [
  ...
  {
    children: [
      { children: [], path: '/contact/email' },
      { children: [], path: '/contact/phone' },
    ],
    path: '/contact',
  },
  {
    children: [
      { children: [], path: '/services/design' },
      {
        children: [{ children: [], path: '/services/development/web' }],
...
describe('menu Finder Tests', () => {
  ...
  it('handles menu items without children', () => {
    const menu = findMenuByPath(
      [{ children: undefined, path: '/only' }] as any[],
      '/only',
    );
    expect(menu).toBeDefined();
