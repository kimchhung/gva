...
const _t = (item: MenuRecordRaw): string => {
  return item.meta?.titleEn
    ? translateMetaTitle(item.meta, { $t, $locale }).title
    : $t(item.meta?.translateKey ?? item.name);
};
...
async function handleEnter() {
  ...
  if (to) {
    searchHistory.value.unshift(to);
    handleClose();
    await nextTick();
    router.push(to.path);
  }
...
onMounted(() => {
  searchItems.value = mapTree(props.menus, (item) => {
    if (item.meta) {
      const tranlatedMeta = translateMetaTitle(item.meta, { $t, $locale });
      return {
        ...item,
        meta: {
          ...item.meta,
    ...
    <div class="!flex h-full justify-center px-4 sm:max-h-[450px]">
      ...
      <ul v-show="searchResults.length > 0" class="w-full">
        ...
        >
          ...
          <span class="flex-1">{{ _t(item) }}</span>
          <div
