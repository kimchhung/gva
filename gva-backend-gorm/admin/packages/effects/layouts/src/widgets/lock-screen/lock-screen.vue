...
onMounted(() => {
  // Get a reference to the #layout element
  const layoutElement = document.querySelector('#layout');

  // Ensure that the #layout element exists
  if (!layoutElement) {
    return;
  }

  // Set up the MutationObserver to detect when the lock screen is removed
  const observer = new MutationObserver((mutationsList: MutationRecord[]) => {
    mutationsList.forEach((mutation) => {
      mutation.removedNodes.forEach((removedNode) => {
        const element = removedNode as HTMLElement;
        // Check if the removed element is the lock screen
        if (element === lockScreenRef.value) {
          emit('toLogin');
        }
      });
    });
  ...
  // Observe only the child list of the #layout element
  observer.observe(layoutElement, { childList: true });
...
<template>
  <div ref="lockScreenRef" class="bg-background fixed z-[2000] size-full">
