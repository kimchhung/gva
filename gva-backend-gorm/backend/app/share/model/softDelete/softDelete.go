...
// MarshalJSON converts the DeletedAt to a JSON string in time.Time format
...
// UnmarshalJSON parses a JSON string to DeletedAt in time.Time format
...
// ToTime converts DeletedAt to time.Time
func (d DeletedAt) ToTime() time.Time {
	if d == 0 {
		return time.Time{}
	}
	return time.Unix(int64(d)/1000, (int64(d)%1000)*1e6)
}

// FromTime sets DeletedAt from time.Time
func (d *DeletedAt) FromTime(t time.Time) {
	*d = DeletedAt(t.Unix()*1000 + int64(t.Nanosecond()/1e6))
}
