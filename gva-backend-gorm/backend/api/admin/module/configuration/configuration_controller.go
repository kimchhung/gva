...
import (
	"backend/api/admin/module/configuration/dto"
...
func (con *ConfigurationController) Create() *ctr.Route {
	return ctr.POST("/").Do(func() []ctr.H {
		...
		return []ctr.H{
			permission.RequireAnyWithSuper(permission.ConfigurationAdd),
			request.Validate(
...
func (con *ConfigurationController) Get() *ctr.Route {
	return ctr.GET("/:id").Do(func() []ctr.H {
		...
		return []ctr.H{
			permission.RequireAnyWithSuper(permission.ConfigurationView),
			request.Validate(
...
func (con *ConfigurationController) Update() *ctr.Route {
	return ctr.PUT("/:id").Do(func() []ctr.H {
		...
		return []ctr.H{
			permission.RequireAnyWithSuper(permission.ConfigurationEdit),
			request.Validate(
				request.ParamsParser(params),
				request.BodyParser(body),
			),
...
func (con *ConfigurationController) Delete() *ctr.Route {
	return ctr.DELETE("/:id").Do(func() []ctr.H {
		...
		return []ctr.H{
			permission.RequireAnyWithSuper(permission.ConfigurationDelete),
			request.Validate(
				request.ParamsParser(params),
			),
			func(c echo.Context) error {
