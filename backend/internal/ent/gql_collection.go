...
// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (mt *MyTodoQuery) CollectFields(ctx context.Context, satisfies ...string) (*MyTodoQuery, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return mt, nil
	}
...
func (mt *MyTodoQuery) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(mytodo.Columns))
		selectedFields = []string{mytodo.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "createdAt":
			if _, ok := fieldSeen[mytodo.FieldCreatedAt]; !ok {
				selectedFields = append(selectedFields, mytodo.FieldCreatedAt)
				fieldSeen[mytodo.FieldCreatedAt] = struct{}{}
			}
		case "updatedAt":
			if _, ok := fieldSeen[mytodo.FieldUpdatedAt]; !ok {
				selectedFields = append(selectedFields, mytodo.FieldUpdatedAt)
				fieldSeen[mytodo.FieldUpdatedAt] = struct{}{}
			}
		case "deletedAt":
			if _, ok := fieldSeen[mytodo.FieldDeletedAt]; !ok {
				selectedFields = append(selectedFields, mytodo.FieldDeletedAt)
				fieldSeen[mytodo.FieldDeletedAt] = struct{}{}
			}
		case "name":
			if _, ok := fieldSeen[mytodo.FieldName]; !ok {
				selectedFields = append(selectedFields, mytodo.FieldName)
				fieldSeen[mytodo.FieldName] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
type mytodoPaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []MyTodoPaginateOption
}

func newMyTodoPaginateArgs(rv map[string]any) *mytodoPaginateArgs {
	args := &mytodoPaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &MyTodoOrder{Field: &MyTodoOrderField{}, Direction: entgql.OrderDirectionAsc}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithMyTodoOrder(order))
			}
		case *MyTodoOrder:
			if v != nil {
				args.opts = append(args.opts, WithMyTodoOrder(v))
			}
		}
	}
	if v, ok := rv[whereField].(*MyTodoWhereInput); ok {
		args.opts = append(args.opts, WithMyTodoFilter(v.Filter))
	}
	return args
}

// CollectFields tells the query-builder to eagerly load connected nodes by resolver context.
func (mt *MyTodo1Query) CollectFields(ctx context.Context, satisfies ...string) (*MyTodo1Query, error) {
	fc := graphql.GetFieldContext(ctx)
	if fc == nil {
		return mt, nil
	}
...
func (mt *MyTodo1Query) collectField(ctx context.Context, oneNode bool, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	path = append([]string(nil), path...)
	var (
		unknownSeen    bool
		fieldSeen      = make(map[string]struct{}, len(mytodo1.Columns))
		selectedFields = []string{mytodo1.FieldID}
	)
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		switch field.Name {
		case "createdAt":
			if _, ok := fieldSeen[mytodo1.FieldCreatedAt]; !ok {
				selectedFields = append(selectedFields, mytodo1.FieldCreatedAt)
				fieldSeen[mytodo1.FieldCreatedAt] = struct{}{}
			}
		case "updatedAt":
			if _, ok := fieldSeen[mytodo1.FieldUpdatedAt]; !ok {
				selectedFields = append(selectedFields, mytodo1.FieldUpdatedAt)
				fieldSeen[mytodo1.FieldUpdatedAt] = struct{}{}
			}
		case "deletedAt":
			if _, ok := fieldSeen[mytodo1.FieldDeletedAt]; !ok {
				selectedFields = append(selectedFields, mytodo1.FieldDeletedAt)
				fieldSeen[mytodo1.FieldDeletedAt] = struct{}{}
			}
		case "name":
			if _, ok := fieldSeen[mytodo1.FieldName]; !ok {
				selectedFields = append(selectedFields, mytodo1.FieldName)
				fieldSeen[mytodo1.FieldName] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
...
type mytodo1PaginateArgs struct {
	first, last   *int
	after, before *Cursor
	opts          []MyTodo1PaginateOption
}

func newMyTodo1PaginateArgs(rv map[string]any) *mytodo1PaginateArgs {
	args := &mytodo1PaginateArgs{}
	if rv == nil {
		return args
	}
	if v := rv[firstField]; v != nil {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &MyTodo1Order{Field: &MyTodo1OrderField{}, Direction: entgql.OrderDirectionAsc}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithMyTodo1Order(order))
			}
		case *MyTodo1Order:
			if v != nil {
				args.opts = append(args.opts, WithMyTodo1Order(v))
			}
		}
	}
	if v, ok := rv[whereField].(*MyTodo1WhereInput); ok {
		args.opts = append(args.opts, WithMyTodo1Filter(v.Filter))
	}
	return args
}
