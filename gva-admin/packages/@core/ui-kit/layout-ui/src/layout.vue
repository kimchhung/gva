...
const realLayout = computed(() => (props.isMobile ? 'sidebar-nav' : props.layout));
 ...
 * 是否侧边混合模式
 */
const isSidebarMixedNav = computed(() => realLayout.value === 'sidebar-mixed-nav');
...
const headerWrapperStyle = computed((): CSSProperties => {
  ...
  return {
    ...
    top: headerIsHidden.value || fullContent.value ? `-${headerWrapperHeight.value}px` : 0,
...

{
  ...
  watch(
    () => scrollY.value,
    () => {
      if (props.headerMode !== 'auto-scroll' || isMixedNav.value || fullContent.value) {
        return;
      }
...
<template>
  <div class="relative flex min-h-full w-full">
    ...
    <div class="flex flex-1 flex-col overflow-hidden transition-all duration-300 ease-in">
      <div :style="headerWrapperStyle" class="overflow-hidden transition-all duration-200">
        <LayoutHeader
          v-if="headerVisible"
